...
import {coaches, kpis, metrics} from 'appData';
...
const CoachDetails = ({coachId}) => {
  ...
  const coachData =
    !isNil(coachId) && coaches.find((team) => team.id === coachId);
  const playerKpisIds = coachData.children.ids;
  ...
  return (
    <AppGridContainer>
      ...
      <Grid item xs={12} md={3}>
        <VideoCall playerData={coachData} />
      </Grid>
      ...
      <Grid item xs={12} md={6}>
        ...
        >
          <ActionsList
            items={coachData.actions.map((action) => ({
              id: action.id,
      ...
      <Grid item xs={12} md={6}>
        ...
        >
          <ActionsList
            items={coachData.recommendations.map((action) => ({
              id: action.id,
              primaryText: action.name,
              secondaryText: action.description,
            }))}
